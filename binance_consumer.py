        ...
        for message in consumer:
            ...
            try:
                if message['eventType'] == "httpEvent":
                    ...
                    orderDetail = getBinanceTradeOrder(clientorderid = message['clientOrderId'])
                    ...
                    if message['status'] == 'PARTIALLY_FILLED':
                        ...
                        amount = Decimal(orderDetail['amount']) - Decimal(message['executedQty'])
                        freezed = Decimal(orderDetail['freezed']) - Decimal(message['cummulativeQuoteQty'])
                        updateBinanceTradeOrder(
                            clientorderid = message['clientOrderId'],
                    ...
                    if message['status'] == "CANCELED":
                        ...
                        print(Fore.GREEN+f"Consumer_process_event eventType - {message['eventType']} - eventName : {message['eventName']} - data -> {message}"+Fore.RESET)
                        clientorderid = None
                        if 'origClientOrderId' in  message:
                            clientorderid = message['origClientOrderId']
                        else:
                            clientorderid = message['clientOrderId'] 
                            
                        order = getBinanceTradeOrder(clientorderid=clientorderid)
                        status = None
                ...
                if  message['eventType'] == "wsocketEvent":
                    orderID = message['c']
                    orderDetail = getBinanceTradeOrder(clientorderid = message['c'])
                    if message['X'] == "NEW":
                    ...
                    if message['X'] == 'PARTIALLY_FILLED':
                        ...
                        if orderDetail['trandata'] is not None:
                            trandata = json.loads(orderDetail['trandata'])
                        ...
                        amount = Decimal(orderDetail['amount']) - Decimal(message['l'])
                        freezed = Decimal(orderDetail['freezed']) - Decimal(message['Y'])
